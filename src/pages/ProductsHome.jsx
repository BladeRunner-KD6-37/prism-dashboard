...
function ProductsHome() {
  ...
  // Track if intro was already completed when component mounted (to skip intro on navigation back)
  const initialIntroCompleted = useRef(introCompleted)
  ...
  useGSAP(() => {
    if (initialIntroCompleted.current) return
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          if (introRef.current) {
            const imgElements = introRef.current.querySelectorAll('.postcard-img')
    ...
    // Fade out all intro elements at the end
    tl.to(
      ['#postcard', '#we-close', '#the-gap'],
      { opacity: 0, ease: 'none' },
      0.8
    )
    ...
    <div ref={containerRef} className="w-full">
      ...
      {!initialIntroCompleted.current && (
        ...
        >
          ...
          <div
            id="scroll-indicator"
            className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-zinc-500 text-xs tracking-widest font-semibold z-30 pointer-events-none select-none"
          >
      ...
      <div
        ...
        style={{
          ...
          transform: introCompleted ? 'translateY(0)' : 'translateY(30px)',
